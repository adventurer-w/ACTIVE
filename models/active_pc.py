...
class ACTIVEPC(nn.Module):
    ...
    def forward(self, input):                     #torch.Size([14, 24, 2048, 3])                                                                                            # [B, L, N, 3]
        device = input.get_device()
        
        xyzs, features = self.tube_embedding1(input)           #torch.Size([b, t, n, 3])            torch.Size([b, t, c=128, n=384])                                # [B, L, n, 3], [B, L, C, n] 
        
        xyzs1, features1 = xyzs[1], features[1]
        xyzs2, features2 = xyzs[2], features[2]
        xyzs3, features3 = xyzs[3], features[3]

        features1 = features1.permute(0, 1, 3, 2)   # [b, t, n, c]
        ...
        features2 = features2.permute(0, 1, 3, 2)   # [2, 10, 96, 256]
        ...
        output2 = self.gap2(output2).squeeze(-1)  # Global Average Pooling
